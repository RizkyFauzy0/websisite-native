...
<?php endif; ?>
<div class="flex justify-between items-center mb-6"><div><h1 class="text-3xl font-bold text-gray-800">Galeri Foto</h1></div>
...
<div class="bg-white rounded-xl shadow-md p-6"><p class="text-gray-600">Modul Galeri Foto siap digunakan. Silakan tambah data melalui tombol di atas.</p>
...
<button onclick="deleteData(<?= $item->id ?>, '<?= BASE_URL ?>/admin/galeri-foto/delete/<?= $item->id ?>')" class="text-red-600"><i class="fas fa-trash"></i></button></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
</div>
...
<form action="<?= BASE_URL ?>/admin/galeri-foto/store" method="POST" enctype="multipart/form-data"><div class="p-4">
<p class="text-gray-600">Form Galeri Foto - Silakan isi data yang diperlukan</p></div>
