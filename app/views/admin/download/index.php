...
<?php endif; ?>
<div class="flex justify-between items-center mb-6"><div><h1 class="text-3xl font-bold text-gray-800">File Download</h1></div>
...
<div class="bg-white rounded-xl shadow-md p-6"><p class="text-gray-600">Modul File Download siap digunakan. Silakan tambah data melalui tombol di atas.</p>
<?php if(!empty($download_list)): ?><div class="mt-4"><table class="w-full"><tbody><?php foreach($download_list as $index => $item): ?><tr class="border-b"><td class="py-2"><?= $index + 1 ?></td><td class="py-2">
...
<button onclick="deleteData(<?= $item->id ?>, '<?= BASE_URL ?>/admin/download/delete/<?= $item->id ?>')" class="text-red-600"><i class="fas fa-trash"></i></button></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
</div>
...
<form action="<?= BASE_URL ?>/admin/download/store" method="POST" enctype="multipart/form-data"><div class="p-4">
<p class="text-gray-600">Form File Download - Silakan isi data yang diperlukan</p></div>
