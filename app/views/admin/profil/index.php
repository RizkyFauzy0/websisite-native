...
<?php endif; ?>
<div class="flex justify-between items-center mb-6"><div><h1 class="text-3xl font-bold text-gray-800">Profil Sekolah</h1></div>
...
<div class="bg-white rounded-xl shadow-md p-6"><p class="text-gray-600">Modul Profil Sekolah siap digunakan. Silakan tambah data melalui tombol di atas.</p>
<?php if(!empty($profil_list)): ?><div class="mt-4"><table class="w-full"><tbody><?php foreach($profil_list as $index => $item): ?><tr class="border-b"><td class="py-2"><?= $index + 1 ?></td><td class="py-2">
<button onclick='editData(<?= json_encode($item) ?>)' class="text-blue-600 mr-3"><i class="fas fa-edit"></i></button>
<button onclick="deleteData(<?= $item->id ?>, '<?= BASE_URL ?>/admin/profil/delete/<?= $item->id ?>')" class="text-red-600"><i class="fas fa-trash"></i></button></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
</div>
...
<form action="<?= BASE_URL ?>/admin/profil/store" method="POST" enctype="multipart/form-data"><div class="p-4">
<p class="text-gray-600">Form Profil Sekolah - Silakan isi data yang diperlukan</p></div>
